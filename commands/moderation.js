module.exports = (bot, pool, canModerate, isOwner) => {
    ...
    bot.command("ban", async (ctx) => {
        ...
        if (!ctx.message.reply_to_message) {
            return ctx.reply("❗ Reply to the user's message with /ban");
        }
        ...
        try {
            ...
            await pool.query(
                `INSERT INTO logs (chat_id,user_id,admin_id,action)
                 VALUES ($1,$2,$3,$4)`,
    ...
    bot.command("kick", async (ctx) => {
        ...
        if (!ctx.message.reply_to_message) {
            return ctx.reply("❗ Reply to the user's message with /kick");
        }
